...
export class RedisService {
  ...
  public async getList(key: KeyType) {
    const res = await this.pubClient.lrange(key, 0, -1);
    return await JSON.stringify(res);
  }
  
  public async getAllByKey(key: string) {
    const res = await this.pubClient.keys(key);
    return await res//JSON.parse(res);
  }
}